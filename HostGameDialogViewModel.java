...
import com.himelz.nexusboard.network.GameServer;
...
public class HostGameDialogViewModel {
    ...
    public void startServer() {
        ...
        new Thread(() -> {
            try {
                server = new GameServer(Integer.parseInt(port.get()));
                server.start(); // blocking call, so run inside thread
            } catch (Exception e) {
                Platform.runLater(() -> {
                    statusMessage.set("Failed to start server: " + e.getMessage());
                });
            }
        }).start();
